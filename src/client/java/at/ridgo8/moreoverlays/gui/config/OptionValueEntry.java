...
public abstract class OptionValueEntry<V> extends ConfigOptionList.OptionEntry {
    ...
    protected final ConfigSpec.ConfigValue<V> value;
    ...
    public OptionValueEntry(ConfigOptionList list, ConfigSpec.ConfigValue<V> confValue) {
        super(list);
        ...
        this.defaultValue = this.value.getDefault();
        ...
        if (this.value.getComment() != null) {
            lines = this.value.getComment().split("\\n");
    ...
    protected void updateValue(@Nullable V value) {
        this.valid = value != null && this.value.test(value);
