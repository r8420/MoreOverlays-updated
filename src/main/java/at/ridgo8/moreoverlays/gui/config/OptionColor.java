...
public class OptionColor extends OptionValueEntry<Integer> {
    ...
    public OptionColor(ConfigOptionList list, ForgeConfigSpec.IntValue valSpec, ForgeConfigSpec.ValueSpec spec) {
        ...
        this.overrideUnsaved(((ForgeConfigSpec.IntValue) this.value).get());
    }
    ...
    protected void renderControls(GuiGraphics guiGraphics, int rowTop, int rowLeft, int rowWidth, int itemHeight, int mouseX, int mouseY,
        ...
        int rgb = ((this.newValue != null && this.isValid()) ? this.newValue : ((ForgeConfigSpec.IntValue) this.value).get());
        rgb = rgb & 0xFFFFFF;
