...
public class ChunkBoundsRenderer {
    ...
    public static void renderOverlays(PoseStack matrixstack) {
        ...
        // The Fabric version uses a render_chunkThick flag; NeoForge version infers thick
        // rendering from the configured line width (matching the light overlay behaviour).
        double desiredPixelWidth = Math.max(0.1, Config.render_spawnLineWidth.get());
        boolean useDebugLines = desiredPixelWidth <= 2.0;
    ...
    private static void addThickLine(VertexConsumer renderer, Matrix4d matrix4d, Vector3fc cameraLook,
    ...
    private static void renderEdgeThick(PoseStack matrixstack, double x, double z, double h3, double h, int color,
                                        Camera camera, Vector3fc look, double desiredPixelWidth) {
        Matrix4d matrix4d = new Matrix4d();
        matrixstack.last().pose().get(matrix4d);
        net.minecraft.client.renderer.MultiBufferSource.BufferSource bufferSource = Minecraft.getInstance().renderBuffers().bufferSource();
    ...
    private static void renderGridThick(PoseStack matrixstack, double x0, double y0, double z0, double x1, double y1, double z1,
                                        double step, int color, Camera camera, Vector3fc look, double desiredPixelWidth) {
        Matrix4d matrix4d = new Matrix4d();
        matrixstack.last().pose().get(matrix4d);
        net.minecraft.client.renderer.MultiBufferSource.BufferSource bufferSource = Minecraft.getInstance().renderBuffers().bufferSource();
