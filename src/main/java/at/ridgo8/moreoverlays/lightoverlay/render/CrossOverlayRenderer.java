...
public class CrossOverlayRenderer implements ILightRenderer {
    ...
    private static BufferBuilder renderer = tess.getBuilder();
    ...
    private static void drawVertex(Matrix4f matrix, double x, double y, double z, float r, float g, float b) {
        ...
        renderer.vertex(tx, ty, tz).color(r, g, b, 1).endVertex();
    }
    ...
    private static void addThickLine(Matrix4f currentMatrix, Vector3f cameraLook,
        ...
        renderer.vertex((float)(a1x - cameraX), (float)(a1y - cameraY), (float)(a1z - cameraZ)).color(r, g, b, 1).endVertex();
        renderer.vertex((float)(b1x - cameraX), (float)(b1y - cameraY), (float)(b1z - cameraZ)).color(r, g, b, 1).endVertex();
        renderer.vertex((float)(b2x - cameraX), (float)(b2y - cameraY), (float)(b2z - cameraZ)).color(r, g, b, 1).endVertex();

        renderer.vertex((float)(a1x - cameraX), (float)(a1y - cameraY), (float)(a1z - cameraZ)).color(r, g, b, 1).endVertex();
        renderer.vertex((float)(b2x - cameraX), (float)(b2y - cameraY), (float)(b2z - cameraZ)).color(r, g, b, 1).endVertex();
        renderer.vertex((float)(a2x - cameraX), (float)(a2y - cameraY), (float)(a2z - cameraZ)).color(r, g, b, 1).endVertex();
    }
    ...
    public void renderOverlays(ILightScanner scanner, PoseStack poseStack) {
        // Adapter to Matrix4f and BufferBuilder behavior in 1.20.1 Forge
        Minecraft.getInstance().getTextureManager().bindForSetup(BLANK_TEX);

        RenderSystem.enableDepthTest();
        // Do not change blend state to avoid leaking into world rendering
        RenderSystem.setShader(net.minecraft.client.renderer.GameRenderer::getPositionColorShader);

        // Always avoid writing to depth so overlay renders consistently
        ...
        RenderSystem.lineWidth(useDebugLines ? (float) configuredWidth : 1.0f);
        if (useDebugLines) {
            RenderSystem.enableCull();
        } else {
            RenderSystem.disableCull();
        }
        renderer.begin(useDebugLines ? VertexFormat.Mode.DEBUG_LINES : VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_COLOR);
        ...
        try {
            for (Pair<BlockPos, Byte> entry : scanner.getLightModes()) {
                ...
                // No aggressive view-angle culling to ensure visibility

                if (useDebugLines) {
                    if (mode == 1)
                        renderCross(currentMatrix, cameraX, cameraY, cameraZ, bp, nr, ng, nb);
                    else if (mode == 2)
                        renderCross(currentMatrix, cameraX, cameraY, cameraZ, bp, ar, ag, ab);
                } else {
